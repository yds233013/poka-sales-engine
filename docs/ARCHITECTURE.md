...
**Freight** (`engines/freight.ts`) — rated per leg, because a split shipment really does incur two
base charges. If ground misses the customer's date, service upgrades one step at a time and the
quote reports `expedited: true`. If even air misses it, the quote says so rather than quietly
failing.

**Margin** (`engines/margin.ts`) — freight is a cost of sale, not a marked-up revenue line. A
long-haul split therefore genuinely erodes deal margin, which is the behaviour the approval engine
needs to see. Margin percent is taken against total invoiced value (goods + freight), which is how
the deal lands on the P&L; margin before freight is reported separately for the operator.
